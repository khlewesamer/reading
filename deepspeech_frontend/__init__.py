...
from flask import Flask, flash, render_template, request,response, redirect, url_for, send_from_directory, make_response, jsonify
...
def api_transcribe():
    ...
    if 'file' not in request.files:
        return make_response(jsonify({'error': request.files}), 400)
    file = request.files['file']
